...
|------|------|------|
| 1 | `shared.css` | 共用样式 |
| 2 | `01-cover.html` | 封面：背景图+渐变蒙层+大字标题+SVG 路线图 |
| 3~N+2 | `02-day1.html` ~ `{N+1}-dayN.html` | N 天行程：每天一张，统一模板 |
| N+3 | `{N+2}-food.html` | 美食合集：餐厅卡片网格，暖色背景 |
| N+4 | `{N+3}-budget.html` | 预算一览：横向条形图+省钱 tips |
| N+5 | `{N+4}-ending.html` | 结尾互动：情感金句+数据回顾+CTA |
| — | `index.html` | 导航页（可选） |

**页码标注**：每张卡片显示 `X/(N+4)`，进度点共 N+4 个。封面为 `01`，结尾为 `N+4`。
...
输出前确认：
- [ ] N+5 个文件全部生成（shared.css + N 张 day 卡片 + 封面/美食/预算/结尾 4 张 + index.html）
